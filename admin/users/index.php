...
<?php
    $formby = $form.'/users';
    $filter_as = strtolower($index['page'].'_userlist_as');
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-8 col-sm-8 col-md-8 col-lg-6 filter-keyword">
                            <div class="mc-field-group input-group form-floating mb-1">
                                ...
                                <button type="button" class="btn btn-purple btn-adding" title="Create New" onclick="manage_events('new', { 'link':'<?=$link?>' });"><i class="uil uil-plus"></i><span> ผู้ใช้ใหม่</span></button>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <select name="condition[role]" class="form-select mb-1">
                                <option value="ALL"<?=((!isset($filter['condition']['role'])||$filter['condition']['role']=='ALL')?' selected':null)?>>แสดงทุกบัญชี</option>
                                <option value="ADMIN"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='ADMIN')?' selected':null)?>>[ADMIN] ผู้ดูแลระบบ</option>
                                <option value="STAFF"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='STAFF')?' selected':null)?>>[STAFF] เจ้าหน้าที่</option>
                                <option value="USER"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='USER')?' selected':null)?>>[USER] ผู้ใช้ทั่วไป</option>
                            </select>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <select name="condition[cmu]" class="form-select mb-1">
                                <option value="ALL"<?=((!isset($filter['condition']['cmu'])||$filter['condition']['cmu']=='ALL')?' selected':null)?>>แสดงทุกประเภท</option>
                                <option value="CMU"<?=((isset($filter['condition']['cmu'])&&$filter['condition']['cmu']=='CMU')?' selected':null)?>>มีบัญชี CMU Mail</option>
                                <option value="NOT"<?=((isset($filter['condition']['cmu'])&&$filter['condition']['cmu']=='NOT')?' selected':null)?>>ไม่มีบัญชี CMU Mail</option>
                            </select>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <select name="condition[status]" class="form-select mb-1">
                                ...
                                <option value="ST1"<?=((isset($filter['condition']['status'])&&$filter['condition']['status']=='ST1')?' selected':null)?>>พร้อมใช้งาน</option>
                                <option value="ST2"<?=((isset($filter['condition']['status'])&&$filter['condition']['status']=='ST2')?' selected':null)?>>ระงับใช้งาน</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="type">บัญชี</th>
    ...
    function manage_events(action, params){
        if(action=='new'){
            ...
            swal({
                ...
                'html' : '<div class="fs-24 text-red on-font-primary mb-2">'+params.email+'</div><div>ยืนยันลบผู้ใช้นี้ ใช่ หรือ ไม่ ?</div>',
